...
def main(args):
    ...
    for time in range(6, 19) :
        ...
        if args.t == "now":
            args.t = datetime.datetime.now()
            args.t = (str(args.t)[0:10])

        date_time = args.t + f" {time}:00:00"
        try:
            date_time = datetime.datetime.strptime(date_time,'%Y-%m-%d %H:%M:%S.%f') #with microseconds
        except:
            try:
                date_time = datetime.datetime.strptime(date_time,'%Y-%m-%d %H:%M:%S.') #without microseconds
            except:
                date_time = datetime.datetime.strptime(date_time,'%Y-%m-%d %H:%M:%S')

        az, zen, ra, dec, h = sunpos(date_time, args.lat, args.lon, args.elev, args.temp, args.p, args.dt, args.rad)
        # if args.csv:
        #     #machine readable
        ...
        ''' The h parameter calculates the zenith angle (angle of sun from the vertical axis), which is used to track the movement in xy plane.
            The elevation angle = 90 - zenith angle (h). While the gps coordinates can be used for observer. So by adding the equation, the elevation angle can be calculated as well.
            The coordinates for the camera/screen can be treated as coordinates for observer. '''

        print("Computing sun position at T = {t} + {dt} s".format(t=date_time, dt=args.dt))
        ...
        if args.dir=='East':
            print(f"[billboard][{date_time}] = {East}")
            print(f"[camera][{date_time}] = {West}\n")

        elif args.dir=='West':
            print(f"[billboard][{date_time}] = {West}")
            print(f"[camera][{date_time}] = {East}\n")  

        elif args.dir=='North':      
            print(f"[billboard][{date_time}] = {North}")
            print(f"[camera][{date_time}] = {South}\n")  

        elif args.dir=='South':      
            print(f"[billboard][{date_time}] = {South}")
            print(f"[camera][{date_time}] = {North}\n")  

        else:
            print("Input Valid Camera Direction")
            break         
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d',dest='dir',type=str, default='West', help='direction of Camera facing')
